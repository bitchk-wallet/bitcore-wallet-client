...
var Bitcore_ = Common.Utils.getBitCoreAll();
...
var BASE_URL = 'https://bws.bitchk.com/bws/api';
...
API.prototype.checkCoin = function(coin) {
    if (!Bitcore.Networks.get(coin, "coin"))
        throw new Error("invalid coin [" + coin + " ]");
}
API.prototype.checkNetwork = function(network) {
    if (!Bitcore.Networks.get(network, "name"))
        throw new Error("invalid network [" + network + " ]");
}
API.prototype.checkCoinNetwork = function(coin, network) {
    this.checkCoin(coin);
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
    ...
    opts = opts || {};
    this.credentials = Credentials.createWithMnemonic(opts.coin || 'btc',
...
API.prototype._doRequest = function(method, url, args, useSession, cb) {
    var self = this;
    //console.log("doRequest", method, url, args, self.baseUrl + url);
...
API.prototype.getFeeLevels = function(coin, network, cb) {
    ...
    $.checkArgument(coin || !Bitcore.Networks.get(coin, "coin"));
    $.checkArgument(network || !Bitcore.Networks.get(network, "network"));

    self._doGetRequest('/v2/feelevels/?coin=' + (coin || 'btc') + '&network=' + (network || 'livenet'), function(err, result) {
        if (err) {
            log.error("request", '/v2/feelevels/?coin=' + (coin || 'btc') + '&network=' + (network || 'livenet'));
            log.error("GEt Fee LEVEL eRROR", err);
            return cb(err);
        }
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
    ...
    this.checkCoin(opts.coin);
    this.checkNetwork(opts.network);
    ...
    if (network != self.credentials.network) {
        return cb(new Error('Existing keys were created for a different network. req:' + network + ",cred:", self.credentials.network));
    }
