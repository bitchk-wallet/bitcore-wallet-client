...
function _checkCoin(coin) {
    var network = Bitcore.Networks.get(coin, "coin");
    if (!network)
        throw new Error("Invalid coin [" + coin + "]");
};

function _checkNetwork(network) {
    var network = Bitcore.Networks.get(network, "name");
    if (!network)
...
var wordsForLang = {
    ...
    'ko': Mnemonic.Words.KOREAN,
};

Credentials.createWithMnemonic = function(coin, network, passphrase, language, account, opts) {
    _checkCoin(coin);
    _checkNetwork(network);
    if (!wordsForLang[language]) throw new Error('Unsupported language [' + language + "]");
...
Credentials.fromExtendedPrivateKey = function(coin, xPrivKey, account, derivationStrategy, opts) {
    ...
    var x = new Credentials();
    x.coin = coin;
    x.network = Bitcore.Networks.get(coin, "coin").name;
    x.xPrivKey = xPrivKey;
    x.account = account;
...
// TODO network param implemnts 
Credentials._getNetworkFromExtendedKey = function(xKey, network) {
...
Credentials.prototype.getKeys = function(password) {
    ...
    var con = unorm.nfkc(keys.mnemonic);
...
Credentials.prototype.getMnemonic = function() {
    ...
    return unorm.nfkc(this.mnemonic);
    // console.log("getMnemonic", this.mnemonic);
